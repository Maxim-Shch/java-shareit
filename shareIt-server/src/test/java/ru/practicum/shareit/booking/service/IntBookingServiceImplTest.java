...
public class IntBookingServiceImplTest {
    ...
    void testGetAllBookingsByUser() {
        Collection<BookingDtoOut> userBookings = bookingService
                .getAllBookingsByUser(booker.getId(), "CURRENT", 0, 10);

        assertNotNull(userBookings);
        assertEquals(2, userBookings.size());

        for (BookingDtoOut dto : userBookings) {
    ...
    void testGetAllBookingsForUserItems() {
        Collection<BookingDtoOut> userBookings = bookingService
                .getBookingsForUserItems(user.getId(), "CURRENT", 0, 10);
